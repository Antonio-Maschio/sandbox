...
class TrackedParticleSimulator:
    ...
    def add_particle(self, x, y, z, mass, parent_ids=None, birth_frame=None):
        ...
        if birth_frame > self.current_frame:
            self.log_warning(f"FUTURE BIRTH - Creating particle with future birth_frame {birth_frame} > current_frame {self.current_frame}")
        ...
        if parent_ids:
            for parent_id in parent_ids:
                ...
                elif parent_particle['active']:
                    self.log_warning(f"ACTIVE PARENT - Parent particle {parent_id} is still active")
    ...
    def process_merge(self, p1, p2):
        """Process a merge event between two particles with validation"""
        ...
        if np.random.random() < self.merge_linking_prob:
            ...
            # DEBUG
            if self.enable_warnings:
                print(f"DEBUG: Linking merge - particle {disappearing_particle['id']} disappears, particle {continuing_particle['id']} continues")
            
            # CRITICAL FIX: Record disappearing particle with MERGE label
            ...
            # CRITICAL FIX: Record continuing particle with POST_MERGE label
            ...
            # DEBUG
            if self.enable_warnings:
                print(f"DEBUG: Normal merge - particles {p1['id']} and {p2['id']} disappear, creating new particle")
            
            # CRITICAL FIX: Record both particles with MERGE label
            ...
            new_id = self.add_particle(new_x, new_y, new_z, total_mass, 
                                     parent_ids=merged_lineage)
            new_particle = next(p for p in self.particles if p['id'] == new_id)
            new_particle['last_event_frame'] = self.current_frame
            
            # DEBUG
            if self.enable_warnings:
                print(f"DEBUG: Created new particle {new_id} from merge")
            
            # CRITICAL FIX: Record new particle with POST_MERGE label
...
# And in the non linking splits the the parent particle  is coexisting in the same frame has the children particles. 
    def process_split(self, particle):
        """Process a split event for a particle with validation"""
        ...
        if np.random.random() < self.split_linking_prob:
            ...
            # DEBUG
            if self.enable_warnings:
                print(f"DEBUG: Linking split - particle {particle['id']} continues, creating new particle")
            
            # CRITICAL FIX: Record continuing particle with POST_SPLIT label first
            ...
            # Record continuing particle with POST_SPLIT
            self.record_particle_state(particle, EventLabel.POST_SPLIT)
            
            # Create new particle with parent lineage (not including continuing particle since it's still active)
            child_lineage = particle['parent_ids'].copy()
            new_id = self.add_particle(new_x, new_y, new_z, new_mass, 
                                     parent_ids=child_lineage)
            new_particle = next(p for p in self.particles if p['id'] == new_id)
            new_particle['last_event_frame'] = self.current_frame
            
            # DEBUG
            if self.enable_warnings:
                print(f"DEBUG: Created new particle {new_id} from linking split")
            
            # Record new particle with POST_SPLIT
            self.record_particle_state(new_particle, EventLabel.POST_SPLIT)
            ...
            # DEBUG
            if self.enable_warnings:
                print(f"DEBUG: Normal split - particle {particle['id']} disappears, creating two new particles")
            
            # CRITICAL FIX: Record parent with SPLIT label first
            ...
            child1_id = self.add_particle(
                ...
                mass1, parent_ids=child_lineage.copy()
            )
            child2_id = self.add_particle(
                particle['x'] + dx/2, particle['y'] + dy/2, particle['z'] + dz/2,
                mass2, parent_ids=child_lineage.copy()
            )
            ...
            # DEBUG
            if self.enable_warnings:
                print(f"DEBUG: Created new particles {child1_id} and {child2_id} from normal split")
            
            # CRITICAL FIX: Record both children with POST_SPLIT
    ...
    def validate_event_labeling(self):
        """Validate that event labeling follows correct sequence"""
        # Check recent merges from this frame
        current_merges = [m for m in self.all_merges if m['frame'] == self.current_frame]
        for merge in current_merges:
            if merge['is_linking']:
                # Linking merge: disappearing particle should have MERGE, continuing should have POST_MERGE
                continuing_id = merge['child_id']
                parent_ids = merge['parent_ids']
                
                # Find which particle disappeared
                disappearing_id = None
                for pid in parent_ids:
                    if pid != continuing_id:
                        disappearing_id = pid
                        break
                
                if disappearing_id:
                    # Check disappearing particle's last label
                    disappearing_particle = next((p for p in self.particles if p['id'] == disappearing_id), None)
                    if disappearing_particle and disappearing_particle['label_history']:
                        last_label = disappearing_particle['label_history'][-1]
                        if last_label != EventLabel.MERGE:
                            self.log_warning(f"MERGE LABEL ERROR - Disappearing particle {disappearing_id} should have MERGE label, got {last_label}")
                
                # Check continuing particle's last label  
                continuing_particle = next((p for p in self.particles if p['id'] == continuing_id), None)
                if continuing_particle and continuing_particle['label_history']:
                    last_label = continuing_particle['label_history'][-1]
                    if last_label != EventLabel.POST_MERGE:
                        self.log_warning(f"POST_MERGE LABEL ERROR - Continuing particle {continuing_id} should have POST_MERGE label, got {last_label}")
            else:
                # Normal merge: both parents should have MERGE, new child should have POST_MERGE
                parent_ids = merge['parent_ids']
                child_id = merge['child_id']
                
                for pid in parent_ids:
                    parent_particle = next((p for p in self.particles if p['id'] == pid), None)
                    if parent_particle and parent_particle['label_history']:
                        last_label = parent_particle['label_history'][-1]
                        if last_label != EventLabel.MERGE:
                            self.log_warning(f"MERGE LABEL ERROR - Parent particle {pid} should have MERGE label, got {last_label}")
                
                child_particle = next((p for p in self.particles if p['id'] == child_id), None)
                if child_particle and child_particle['label_history']:
                    last_label = child_particle['label_history'][-1]
                    if last_label != EventLabel.POST_MERGE:
                        self.log_warning(f"POST_MERGE LABEL ERROR - Child particle {child_id} should have POST_MERGE label, got {last_label}")
        
        # Check recent splits from this frame
        current_splits = [s for s in self.all_splits if s['frame'] == self.current_frame]
        for split in current_splits:
            if split['is_linking']:
                # Linking split: continuing particle should have POST_SPLIT, new particle should have POST_SPLIT
                parent_id = split['parent_id']
                child_ids = split['child_ids']
                
                # Find which particle is the continuing one (should be the parent_id)
                continuing_id = parent_id
                new_id = None
                ...
                if continuing_particle and continuing_particle['label_history']:
                    last_label = continuing_particle['label_history'][-1]
                    if last_label != EventLabel.POST_SPLIT:
                        self.log_warning(f"POST_SPLIT LABEL ERROR - Continuing particle {continuing_id} should have POST_SPLIT label, got {last_label}")
                
                # Check new particle's last label
                if new_id:
                    new_particle = next((p for p in self.particles if p['id'] == new_id), None)
                    if new_particle and new_particle['label_history']:
                        last_label = new_particle['label_history'][-1]
                        if last_label != EventLabel.POST_SPLIT:
                            self.log_warning(f"POST_SPLIT LABEL ERROR - New particle {new_id} should have POST_SPLIT label, got {last_label}")
            else:
                # Normal split: parent should have SPLIT, both children should have POST_SPLIT
                parent_id = split['parent_id']
                child_ids = split['child_ids']
                ...
                if parent_particle and parent_particle['label_history']:
                    last_label = parent_particle['label_history'][-1]
                    if last_label != EventLabel.SPLIT:
                        self.log_warning(f"SPLIT LABEL ERROR - Parent particle {parent_id} should have SPLIT label, got {last_label}")
                
                for cid in child_ids:
                    child_particle = next((p for p in self.particles if p['id'] == cid), None)
                    if child_particle and child_particle['label_history']:
                        last_label = child_particle['label_history'][-1]
                        if last_label != EventLabel.POST_SPLIT:
                            self.log_warning(f"POST_SPLIT LABEL ERROR - Child particle {cid} should have POST_SPLIT label, got {last_label}")
    ...
    def validate_frame_state(self):
        """Validate the overall frame state after update"""
        active_particles = [p for p in self.particles if p['active']]
        
        # Check that all active particles were recorded this frame
        unrecorded = []
        for particle in active_particles:
            if not particle.get('recorded_this_frame', False):
                unrecorded.append(particle['id'])
                
        if unrecorded:
            self.log_warning(f"FRAME INCOMPLETE - Active particles not recorded this frame: {unrecorded}")
            
        # Check for any particles with future birth frames
        future_births = []
        for particle in active_particles:
            if particle['birth_frame'] > self.current_frame:
                future_births.append(particle['id'])
                
        if future_births:
            self.log_warning(f"FUTURE BIRTHS - Active particles with future birth frames: {future_births}")
            
        # Check for trajectory length consistency
        trajectory_errors = []
        for particle in active_particles:
            expected_length = self.current_frame - particle['birth_frame'] + 1
            actual_length = len(particle['trajectory'])
            if actual_length != expected_length:
                trajectory_errors.append(f"{particle['id']}({actual_length}!={expected_length})")
                
        if trajectory_errors:
            self.log_warning(f"TRAJECTORY LENGTH ERRORS - {trajectory_errors}")
        
        # Validate event labeling
        self.validate_event_labeling()
        
        # Validate label sequences
        self.validate_label_sequence()

    def update(self, dt=0.1):
        """Main update function with comprehensive validation"""
        ...
        for p1, p2 in merge_candidates:
            ...
            if np.random.random() < self.merge_prob:
                ...
                # Debug: Check if particles were recorded
                if self.enable_warnings:
                    p1_recorded = p1.get('recorded_this_frame', False)
                    p2_recorded = p2.get('recorded_this_frame', False)
                    if not p1_recorded and p1['active']:
                        self.log_warning(f"MERGE PROCESSING - Particle {p1_id} not recorded after merge but still active")
                    if not p2_recorded and p2['active']:
                        self.log_warning(f"MERGE PROCESSING - Particle {p2_id} not recorded after merge but still active")
        
        # Process splits - get fresh active particles list
        current_active_particles = [p for p in self.particles if p['active']]
        
        for particle in current_active_particles:
            ...
            if (np.random.random() < self.split_prob and 
                ...
                active_D_values = [p['D'] for p in current_active_particles if p['active']]
                if active_D_values:
                    ...
                    if np.random.random() < kinetic_factor:
                        ...
                        # Debug: Check if particle was recorded
                        if self.enable_warnings:
                            if not particle.get('recorded_this_frame', False) and particle['active']:
                                self.log_warning(f"SPLIT PROCESSING - Particle {particle_id} not recorded after split but still active")
        
        # Process normal updates for remaining particles - get fresh active particles list
        final_active_particles = [p for p in self.particles if p['active']]
        
        for particle in final_active_particles:
            # Primary check: Skip if already recorded this frame (most reliable)
            if particle.get('recorded_this_frame', False):
                continue  # Skip particles that were already processed in events
            
            # Secondary check: Skip if in processed_particles set
            if particle['id'] in processed_particles:
                # This should have been caught by the first check, but just in case
                if self.enable_warnings:
                    self.log_warning(f"PROCESSED SKIP - Particle {particle['id']} in processed_particles but not recorded")
                continue
    ...
    def print_summary(self):
        """Print simulation summary including warnings"""
        print(f"\n=== SIMULATION SUMMARY ===")
