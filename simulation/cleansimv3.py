...
class TrackedParticleSimulator:
    ...
    def _process_linking_split(self, particle, mass1, mass2, dx, dy, dz):
        ...
        self.update_existing_particle(particle, new_x, new_y, new_z, 
                                    continuing_mass, particle['parent_ids'])  # Keep original parent_ids
    ...
    def update(self, dt=0.1):
        ...
        # Check for any issues (optional debugging)
        all_deactivated = merged_info['deactivated'].union(split_info['deactivated'])
        all_continuing = merged_info['continuing'].union(split_info['continuing'])
        
        # Check if any particle is marked as both deactivated and continuing
        overlap = all_deactivated.intersection(all_continuing)
        if overlap:
            print(f"Warning: Particles marked as both deactivated and continuing: {overlap}")
